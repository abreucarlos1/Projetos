...
begin
     musica.Caption:='Música: '+table1.Fields[1].Text;
...
begin
     opendialog1.FileName:='';
     opendialog1.Filter:='Arquivos de Seleção|*.slm';
     if opendialog1.Execute then
        begin
...
procedure Tform1.Salvalista;// salva seleção da lista em arquivo
var
   arquivo:textfile;
   vezes:word;
begin
     savedialog1.Filter:='Arquivos de Seleção|*.slm';
...
begin
     ...
     if todatabela.Checked then
        if messagedlg('ATENÇÃO!!! Todos os itens serão EXCLUÍDOS',mtInformation,[mbYes,MbNo],0)=mrYes then
               begin
                    with table1 do
                         begin
                              active:=false;
...
begin
     messagedlg('Não há nenhum item selecionado!!',mtinformation,[mbOk],0);
end;
...
begin
     messagedlg('A tabela já está no inicio!!',mtInformation,[mbOk],0);
end;

procedure tform1.EOFMSG;
begin
     messagedlg('A tabela já está no final!!',mtInformation,[mbOk],0);
end;
...
begin                                  // por 'arquivo'
     ...
     if mpegplayer1.Mode=4 then
        begin
         ...
         if arquivo='' then
            messagedlg('Música não válida...',MTerror,[mbOk],0)
         else
             begin
                  with mpegplayer1 do
                       begin
...
procedure TForm1.pauseClick(Sender: TObject);// botão pausa
begin
     if mpegplayer1.Mode=4 then
        begin
...
procedure TForm1.stopClick(Sender: TObject);// botão stop
begin
     paratocador;
end;
...
procedure TForm1.pathClick(Sender: TObject);// botão caminho do arquivo
begin
     finaliza(indice);
end;

procedure TForm1.adicionaClick(Sender: TObject);// botão adiciona
begin
    adicionar;
end;
...
procedure TForm1.cancelaClick(Sender: TObject);// cancela alterações no BD
begin
     ...
     if (mpegplayer1.Mode=0) or (mpegplayer1.Mode=1) then
        begin
             if grupoprocura.ItemIndex=0 then
                begin
                     ind:=1;
                     editaregistros(ind,'NMINDEX','Nome da Música');
                end;
             if grupoprocura.ItemIndex=1 then
                begin
                     ind:=2;
...
procedure TForm1.DeletarClick(Sender: TObject);// botão delete
begin
     testaradiogroup;
     delet;
end;

procedure TForm1.Adiciona1Click(Sender: TObject);// adiciona musica à listbox
begin
     listbox1.Items.Add(table1.fieldbyname('Nome da Música').AsString);
...
begin
     if (listbox1.ItemIndex<=0) and (listbox2.ItemIndex<=0) then
        messagedlg('Lista já está no inicio',mtInformation,[mbOk],0)
     else
         begin
              listbox1.ItemIndex:=listbox1.ItemIndex-1;
              listbox2.itemindex:=listbox2.itemindex-1;
...
begin
     if (listbox1.ItemIndex>=listbox1.Items.Count-1) and (listbox2.ItemIndex>=listbox2.Items.Count-1) then
        messagedlg('Lista já está no final',mtInformation,[mbOk],0)
     else
         begin
              adicionaitemlista;
         end;
...
begin
     ...
     if grupolista.ItemIndex=1 then
        if listbox1.Items.Count<=0 then
           messagedlg('Não há items na lista',mtError,[mbOk],0)
        else
...
procedure TForm1.salvarClick(Sender: TObject);// botão salvar;
begin
     salvalista;
end;

procedure TForm1.abreClick(Sender: TObject);// botão abrir
begin
     abrirlista;
end;
...
begin
     if (listbox1.Items.Count>0) and (not control) then
        //if messagedlg('Voce deseja tocar música da tabela?',MTinformation,[mbNo,MbYes],0)=mrNo then
             begin
                  if grupolista.ItemIndex=0 then
                     begin
...
procedure TForm1.playClick(Sender: TObject);// botão play
begin
     tocalista;
end;
